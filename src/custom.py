...
"""
@requires_define
async def define_cmd(message: discord.Message) -> str:
    ...
    if cmd in commands:
        ...
        if (commands[cmd][1] & CustomCommandFlags.ADMIN) and not is_admin:
            return f"`{cmd}` was created by a moderator, and only moderators can edit its contents."
        old_response = commands[cmd][0]
...
"""
@requires_admin
async def remove_cmd(message: discord.Message) -> str:
    # First remove the "define" command
    new_cmd = commonbot.utils.strip_words(message.content, 1)
...
"""
@requires_admin
async def limit_cmd(message: discord.Message) -> str:
